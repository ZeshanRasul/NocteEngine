...
float3 ComputerDirectionalLight(Light L, Material mat, float3 normal, float3 toEye)
{
...
float4 ComputeLighting(Light gLights[MaxLights], Material mat, float3 pos, float3 normal, float3 toEye, float3 shadowFactor)
{
    ...
    {
        result += shadowFactor[i] * ComputerDirectionalLight(gLights[i], mat, normal, toEye);
    };
#endif
